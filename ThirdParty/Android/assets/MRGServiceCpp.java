...
public class MRGServiceCpp {
	...
	private static native void onMyComSupportHasError();
	...
	public static void initWithAppidAndSecret(final String appId, final String appSecret) {
		// MRGService.service(_context, mServerDataDelegate, appId, appSecret);
		// MRGSBilling.instance().setDelegate(mBillingDelegate);

		Log.v(LOG_TAG, String.format("MRGServiceCPP:init (%s, %s)", appId, appSecret));
		init(appContext, appId/*"187"*/, appSecret/*"_iRjtRYYCr%x87UTvJV2g#_Db8kV!%4O"*/);
	}
	...
	public static void initUser(final String userId) {
		...
		MRGSUsers.instance().registerNewUser(userId);
		MRGSUsers.instance().authorizationUserWithId(userId);
	}
	...
	/*               Google Analytics               */
	/************************************************/
	public static void sendGAEvent(final String category, final String action, final String label, final String value) {
		Log.v(LOG_TAG, String.format("sendGAEvent('%s', '%s', '%s', %s)", category, action, label, value));
		MRGService.instance().sendGAEvent(category, action, label, Long.valueOf(Integer.parseInt(value)));
	}
